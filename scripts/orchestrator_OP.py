...
import argparse, itertools, json, os, shlex, subprocess
...
OTHER_SCRIPT  = MODELS_DIR / "otherModels" / "main_OP.py"
...
class PreprocCfg:
    ...
    def to_preproc_cli(self, out_npz: Path) -> str:
        args = [
            ...
            f"--l-freq {self.l_freq}" if self.l_freq is not None else "",
...
class DiffEGrid:
    ...
    num_classes: List[int] = field(default_factory=lambda: [5])
    channels: List[int] = field(default_factory=lambda: [64])
...
def main():
    ...
    if cfg and "models" in cfg:
        ...
        diffe_grid = DiffEGrid(
            ...
            subject_id= diffe_cfg.get("subject_id", None),
        ...
        other_grid = OtherGrid(
            ...
            num_classes=other_cfg.get("num_classes", 5),
            subject_id= other_cfg.get("subject_id", None),
            channels=other_cfg.get("channels", 64),
    ...
    for pre_cfg in pre_list:
        ...
        for h in expand_diffe_grid(diffe_grid):
            ...
            diffe_run_dir = ensure_dir(exp_dir / f"DiffE_{variant}" /
                                       f"seed_{h['seed']}_ne{h['num_epochs']}_bt{h['batch_train']}_be{h['batch_eval']}_a{h['alpha']}_ed{h['encoder_dim']}_fd{h['fc_dim']}_dd{h['ddpm_dim']}_nT{h['n_T']}")
            log = diffe_run_dir / "train.log"
            subject_part = f"--subject_id {h['subject_id']} " if h['subject_id'] is not None else ""
        ...
        for o in expand_other_grid(other_grid):
            other_run_dir = ensure_dir(exp_dir / "OtherModels" /
                                       f"seed_{o['seed']}_ne{o['epochs']}_bt{o['batch_train']}_be{o['batch_eval']}_lr{o['lr']}_wd{o['weight_decay']}")
            log = other_run_dir / "benchmark.log"
            models_list = " ".join(o["models"])

            subject_part = f"--subject_id {o['subject_id']} " if o['subject_id'] is not None else ""
            cmd = (
...
'''
python scripts/orchestrator_OP.py --venv-activate /path/to/project/venv/bin/activate --config configs/experimento.yaml
'''
