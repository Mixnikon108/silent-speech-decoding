...
def add_condition_tag(events: np.ndarray) -> np.ndarray:
    """
    Añade la etiqueta 'mod' (0,1,2) que arrastra el último trigger 21-23
    y devuelve solo los trials 31-34 con su mod correspondiente.

    - events: matriz MNE (n_eventos, 3)
    - return : matriz (n_trials, 4)  [sample, 0, code, mod]
    """
    # 1) Generar vector de mods arrastrando 21/22/23
    mod_tag = np.zeros(len(events), dtype=int)
    mod = 0
    for i in range(len(events)):
        if   events[i, 2] == 21: mod = 0
        elif events[i, 2] == 22: mod = 1
        elif events[i, 2] == 23: mod = 2
        mod_tag[i] = mod                    # se arrastra hasta cambiar

    # 2) Máscara de trials 31-34
    trial_mask = np.isin(events[:, 2], [31, 32, 33, 34])

    # 3) Filtrar eventos y mods con la MISMA máscara
    events_kept   = events[trial_mask]
    mods_kept     = mod_tag[trial_mask]

    # 4) Apilar (ya están perfectamente alineados)
    events_tagged = np.hstack((events_kept, mods_kept[:, None]))
