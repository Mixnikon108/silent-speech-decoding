...
def _as_scalar(x: Any):
    # Convierte a tipos JSON-safe (float/int/str/None)
    if isinstance(x, (float, int)): 
        return x
    if isinstance(x, (np.floating, np.integer)):
        return x.item()
    if isinstance(x, torch.Tensor):
        return x.item() if x.ndim == 0 else None  # tensores no escalares se tratan en safe_jsonify
    return x
...
def safe_jsonify(d: dict) -> dict:
    ...
    for k, v in d.items():
        if isinstance(v, dict):
            ...
            out[k] = [
                (_as_scalar(el) if not isinstance(el, (np.ndarray, torch.Tensor)) 
                 else (el.detach().cpu().tolist() if isinstance(el, torch.Tensor) else el.tolist()))
                for el in v
            ]
        elif isinstance(v, torch.Tensor):
            # Tensores → lista (o escalar si 0-D)
            out[k] = v.detach().cpu().item() if v.ndim == 0 else v.detach().cpu().tolist()
...
def array_fingerprint(a: Any, sample_bytes: int = 0) -> str:
    """
    Huella rápida para arrays/tensores grandes.
    - Soporta torch.Tensor, np.ndarray y buffers bytes-like.
    - Para arrays muy grandes, usa solo los primeros `sample_bytes`.
    """
    # Normaliza a bytes
    if isinstance(a, torch.Tensor):
        # Asegura memoria contigua en CPU y convierte a bytes
        arr = a.detach().contiguous().cpu().numpy()
        b = arr.tobytes()
        ...
        # Fallback (poco eficiente): serializa a JSON
        try:
            b = json.dumps(a).encode("utf-8")
        except Exception:
            # último recurso: convierte a np.array y serializa
            b = np.asarray(a).tobytes()

    if sample_bytes and len(b) > sample_bytes:
        b = b[:sample_bytes]
...
class MLPClassifier(nn.Module):
    @_save_args
    def __init__(self, channels: int = 64, n_time: int = 800, num_classes: int = 5):
        super().__init__()
        in_dim = channels * n_time
...
class SimpleCNN(nn.Module):
    ...
    def __init__(self, channels: int = 64, n_time: int = 800, num_classes: int = 5):
        ...
        self.features = nn.Sequential(
            nn.Conv1d(channels, 32, kernel_size=8, padding=4),
...
class EEGNet(nn.Module):
    @_save_args
    def __init__(self, channels: int = 64, n_time: int = 800, num_classes: int = 5, F1: int = 8, D: int = 2, F2_scale: int = 2, dropout: float = 0.25):
        super().__init__()
        F2 = F1 * D * F2_scale
        ...
        with torch.no_grad():
            dummy = torch.zeros(1, 1, channels, n_time)
...
class ShallowConvNet(nn.Module):
    ...
    def __init__(self, channels: int = 64, n_time: int = 800, num_classes: int = 5):
        ...
        self.features = nn.Sequential(
            ...
            nn.Conv2d(40, 40, (channels, 1), groups=40, bias=False),
            nn.BatchNorm2d(40),
...
class DeepConvNet(nn.Module):
    ...
    def __init__(self, channels: int = 64, n_time: int = 800, num_classes: int = 5):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv2d(25, 25, (channels, 1), groups=25, bias=False),
...
def run_benchmark(args):
    ...
    for name in args.models:
        ...
        model = MODEL_ZOO[name](args.channels, args.n_time, args.num_classes).to(device)
        ...
        metrics_payload = {
            ...
            "paths": {"best_ckpt": None, # str(best_ckpt.resolve())
                      "last_ckpt": None, # str(last_ckpt.resolve())
...
def main():
    ...
    data.add_argument("--channels", type=int, default=64)
    data.add_argument("--n_time", type=int, default=800)
    data.add_argument("--num_classes", type=int, default=5)
