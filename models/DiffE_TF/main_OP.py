...
import json, time, hashlib, subprocess, csv
...
def train_epoch(
    *,
    ...
    for x, y in loaders["train"]:
        ...
        with torch.no_grad():
            logits_train = diffe.fc(diffe.encoder(x)[0])
...
# ====== MOD: utilidades de logging/JSON para el orquestador =================
...
def main() -> None:
    ...
    # ===== MOD: salida estructurada para orquestador =====
    parser.add_argument("--results-jsonl", type=str, default=None,
                        help="Ruta a un JSONL global para hacer append del resumen del run")
    # =====================================================
    ...
    diffe = build_diff_e_vit(
        in_ch=args.channels,
        signal_len=X_train.shape[-1],     # 800
        patch_size=25,                    # o expón un flag CLI si quieres
        dim=args.encoder_dim,             # 256 por defecto
        depth=6,
        heads=8,
        latent_dim=args.fc_dim,           # 512
        num_classes=args.num_classes,
        n_feat_decoder=args.ddpm_dim,     # 128 → canales de up1
        ).to(device)
    ...
    # ===== MOD: asegurar que existe el directorio y preparar historial =====
    ...
    for epoch in pbar:
        ...
        # ===== MOD: registrar historia por época =====
        
        lr_ddpm = float(sched_ddpm.get_last_lr()[0])
        lr_diffe = float(sched_diffe.get_last_lr()[0])
        

        history_rows.append({
            "epoch": epoch + 1,
            "train_acc": float(train_acc),
            "loss_ddpm": float(train_loss_ddpm),
            "loss_gap":  float(train_loss_gap),
            "loss_cls":  float(train_loss_cls),
            "val_acc":   float(acc),
    ...
    # ===== MOD: exportar resultados para el orquestador =====
    run_ended = time.time()
    duration_sec = run_ended - run_started

    best_epoch = None
    if history_rows:
        best_epoch = max(history_rows, key=lambda r: r["val_acc"])['epoch']

    result_summary = {
        "timestamp": datetime.now().isoformat(timespec="seconds"),
        "run_id": f"{datetime.now().strftime('%Y%m%d_%H%M%S')}_{short_hash(str(vars(args)))}",
        ...
        "val_best_accuracy": float(best_metrics.get("accuracy", 0.0)),
        "val_best_macro_f1": float(best_metrics.get("macro_f1", 0.0)),
        # Test (del mejor checkpoint)
        "test_accuracy": float(test_report["metrics"]["accuracy"]),
        "test_macro_f1": float(test_report["metrics"]["macro_f1"]),
        "test_precision_macro": float(test_report["metrics"]["macro_precision"]),
        "test_recall_macro": float(test_report["metrics"]["macro_recall"]),
        # Matriz de confusión
        "confusion_matrix": test_report["confusion_matrix"].tolist()
                             if hasattr(test_report["confusion_matrix"], "tolist")
                             else test_report["confusion_matrix"],
        # Misc
    ...
    # Guardar metrics.json (resumen final)
    write_json(exp_dir / "metrics.json", safe_jsonify(result_summary))

    # Append opcional a un JSONL global
    if args.results_jsonl:
        append_jsonl(Path(args.results_jsonl), safe_jsonify(result_summary))

    # Imprimir una sola línea JSON para que el orquestador la capture de stdout
    print("\n[RESULT_JSON]", json.dumps(safe_jsonify(result_summary), ensure_ascii=False))
    # ================================================================
